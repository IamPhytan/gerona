...
void EKF::predict(const std_msgs::Float64MultiArray::ConstPtr &array,
                  double dt) {
  ...
  F(0, 3) =
      dt *
      ((y_ICRl * (Vr - Vl) / (pow((y_ICRl - y_ICRr), 2))) * std::cos(theta) -
       (x_ICR * (Vl - Vr) / (pow(y_ICRl - y_ICRr, 2))) * std::sin(theta));
  F(0, 4) =
      dt *
      ((y_ICRr * (Vl - Vr) / (pow((y_ICRl - y_ICRr), 2))) * std::cos(theta) +
       (x_ICR * (Vl - Vr) / (pow((y_ICRl - y_ICRr), 2))) * std::sin(theta));
  F(0, 5) = -dt * ((Vl - Vr) / (y_ICRl - y_ICRr)) * std::sin(theta);
  F(1, 3) =
      dt *
      ((y_ICRl * (Vr - Vl) / (pow((y_ICRl - y_ICRr), 2))) * std::sin(theta) +
       (x_ICR * (Vl - Vr) / (pow((y_ICRl - y_ICRr), 2))) * std::cos(theta));
  F(1, 4) =
      dt *
      ((y_ICRr * (Vl - Vr) / (pow((y_ICRl - y_ICRr), 2))) * std::sin(theta) -
       (x_ICR * (Vl - Vr) / (pow((y_ICRl - y_ICRr), 2))) * std::cos(theta));
